...
class VehiclePositionsViewModel(application: Application) : AndroidViewModel(application) {
    ...
    init {
        ...
        getApplication<Application>().assets.open("config.properties").use { properties.load(it) }
        ...
        currentJob =
                viewModelScope.launch {
                    try {
                                            ...
                                            async {
                                                try {
                                                    ...
                                                    if (position != null) {
                                                        ...
                                                        position.copy(vehicleId = vehicleId)
                                                    } else {
                                                        Log.w(
                                                                TAG,
                        ...
                        Log.d(TAG, "📊 Actualizando UI con ${positions.size} posiciones")
                        ...
                        positions.forEach { position ->
                            try {
                                Log.d(
                                        TAG,
                                        "💾 Actualizando Room para vehículo ${position.vehicleId}"
                                )
                                ...
                                vehiclePositionDao.insertAll(listOf(vehiclePositionEntity))
                                Log.d(TAG, "✅ Room actualizado para vehículo ${position.vehicleId}")
                            } catch (e: Exception) {
                                Log.e(
                                        TAG,
                                        "❌ Error actualizando Room para vehículo ${position.vehicleId}",
                                        e
                                )
                            }
                        }
                        ...
                        positions.forEach { position ->
                            try {
                                Log.d(
                                        TAG,
                                        "🔥 Iniciando actualización Firestore para vehículo ${position.vehicleId}"
                                )
                                updateOrCreateVehicle(position.vehicleId, position)
                                Log.d(
                                        TAG,
                                        "✅ Firestore actualizado para vehículo ${position.vehicleId}"
                                )
                            } catch (e: Exception) {
                                if (e is kotlinx.coroutines.CancellationException) throw e
                                Log.e(
                                        TAG,
                                        "❌ Error actualizando Firestore para vehículo ${position.vehicleId}",
                                        e
                                )
                            }
                        }
                        ...
                        if (e !is kotlinx.coroutines.CancellationException) {
                            Log.e(TAG, "❌ Error general en refreshVehiclePositions", e)
                            _error.value = e
                        }
                    } finally {
            ...
            if (!vehicleDoc.exists()) {
                val newVehicle =
                        mapOf(
                                ...
                                "current_position" to
                                        mapOf(
                                                "latitude" to position.getLatitudeAsDouble(),
                                                "longitude" to position.getLongitudeAsDouble(),
    ...
    fun getVehiclePositionHistory(vehicleId: String, limit: Int = 100) {
        viewModelScope.launch {
            try {
                ...
                Log.d(TAG, "🔍 Obteniendo historial de coordenadas para vehículo: $vehicleId")
                val historySnapshot =
                        firestore
                                .collection("vehicles")
                                .document(vehicleId)
                ...
                val positions =
                        historySnapshot.documents.mapNotNull { doc ->
                            try {
                                val coordinates = doc.get("coordinates") as? Map<*, *>
                                if (coordinates != null) {
                                    VehiclePosition(
                                            vehicleId = vehicleId,
                                            latitude =
                                                    (coordinates["latitude"] as Double).toString(),
                                            longitude =
                                                    (coordinates["longitude"] as Double).toString(),
                                            timestamp = doc.get("timestamp") as Long
                                    )
                                } else {
                                    Log.w(TAG, "⚠️ Documento sin coordenadas válidas: ${doc.id}")
                                    null
                                }
                            } catch (e: Exception) {
                                Log.e(TAG, "❌ Error parseando documento de historial: ${doc.id}", e)
                                null
                            }
                        }
                ...
                Log.e(TAG, "❌ Error obteniendo historial para vehículo $vehicleId", e)
                _error.value = e
            } finally {
                _isLoading.value = false
            }
